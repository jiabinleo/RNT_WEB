...
window.disasterSelect = (function() {
	...
	//构造器
	function disasterSelect(config) {
		this.mobileSelect;
	...
	disasterSelect.prototype = {
		constructor: disasterSelect,
			...
			for(var i=0; i<wheelsData.length; i++){ //列
				...
				if(_this.jsonType){
					for(var j=0; j<wheelsData[i].data.length; j++){ //行
						tempHTML += '<li data-id="'+wheelsData[i].data[j].id+'">'+wheelsData[i].data[j].text+'</li>';
					}
				}else{
					for(var j=0; j<wheelsData[i].data.length; j++){ //行
		...
		checkCascade: function(){
			...
			if(_this.jsonType){ 
				...
				for(var i=0; i<node.length; i++){
					if('children' in node[i] && node[i].children.length > 0){
						_this.cascade = true;
						break;
					}
				}
		...
		checkArrDeep: function (parent) { //检测子节点深度  修改 displayJson
			...
			if ('children' in parent && parent.children.length > 0) {
				_this.displayJson.push(_this.generateArrData(parent.children)) ; //生成子节点数组
				_this.checkArrDeep(parent.children[0]);//检测下一个子节点
			}
		},
		...
		checkRange: function(index, posIndexArr){
			...
			for (var i = 0; i <= index; i++){
				if (i == 0)
					...
					resultNode = resultNode.children[posIndexArr[i]];
				}
			}
				...
				(function (i) {
					...
					if(_this.wheel[i]){
						...
						for(var j=0; j<_this.displayJson[i].length; j++){ //行
							tempHTML += '<li data-id="'+_this.displayJson[i][j].id+'">'+_this.displayJson[i][j].text+'</li>';
						}
