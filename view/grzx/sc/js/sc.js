(function () {
  var timeOutEvent,
    policyList = {
      ...
      listen: function () {
        ...
        $(document).on("click", ".lis", function () {
          ...
          if ($(this).attr("categorycode") && $(this).attr("categorycode")) {
            if ($(this).attr("categorycode") == "zxtb") {
              window.open(
                "../../homePage/maize/maize.html?id=" +
                $(this).attr("detailid"),
                "_self"
              );
            } else if ($(this).attr("categorycode") == "gqsc") {
              window.open(
                "../../homePage/gqsc/gqxq.html?ids=" + $(this).attr("detailid"),
                "_self"
              );
            }
          }
      ...
      getList() {
        $.ajax({
          url: localhost50010 + "/user/collection/list",
          type: "GET",
          dataType: "json",
          headers: {
            "Content-Type": "application/json"
      ...
      listHTML: function (data) {
        ...
        data.forEach((ele, index) => {
          $("#listHTML").append(
            `<div detailId="${ele.detailId}" class="lis" categoryCode="${
              ele.categoryCode
            }">
                <div class="left lisImg">
                <img src="${imgUrl +
                  ele.icon}" onerror="javascript:this.src='../../../img/error.png'" alt>
            </div>
            <div class="right">
                <h2>${ele.title}</h2>
                <p>${ele.brief}</p>
