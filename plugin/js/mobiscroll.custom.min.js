...
(function (h, f) {
    ...
    var d, e = +new Date(),
        a = {},
        m = h.extend,
        n = document.createElement("modernizr").style,
        k = j(["perspectiveProperty", "WebkitPerspective", "MozPerspective", "OPerspective", "msPerspective"]),
        c = j(["flex", "msFlex", "WebkitBoxDirection"]),
        i = g(),
        b = i.replace(/^\-/, "").replace(/\-$/, "").replace("moz", "Moz");
    ...
    d = h.mobiscroll = h.mobiscroll || {
        ...
        util: {
            ...
            getPosition: function (s, q) {
                var u = window.getComputedStyle ? getComputedStyle(s[0]) : s[0].style,
                    p, r;
                if (k) {
    ...
    h.mobiscroll.classes.Base = function (t, u) {
        var r, A, B, w, y, v, p = h.mobiscroll,
            x = p.util,
            q = x.getCoord,
            z = this;
        z.settings = {};
...
(function (i, n, o, h) {
    var e, m, c = i.mobiscroll,
        ...
        d = j.isString,
        q = j.isOldAndroid,
        a = /(iphone|ipod|ipad).* os 8_/i.test(navigator.userAgent),
    ...
    c.classes.Frame = function (ae, af, B) {
        var M, y, z, ab, W, H, x, r, A, L, P, Z, ai, E, X, K, O, R, ag, aa, C, w, ah, t, V = this,
            U = i(ae),
            G = [],
            Q = {};
        ...
        V.position = function (aG) {
            var ar, aA, av, ao, au, aD, ay, ax, aB, aj, aC, s, aE, ak, aF, az, aJ = 0,
                an = 0,
                aq = {},
                aI = Math.min(r[0].innerWidth || r.innerWidth(), H.width()),
                am = r[0].innerHeight || r.innerHeight(),
                aH = i(o.activeElement);
...
(function (h, k, l, f) {
    ...
    g.Scroller = function (aj, al, F) {
        var ai, W, ad, am, Z, z, ae, D, R, aa, r, O, v, I, ag, an, M, o, T, X, Q, ac = this,
            ab = h(aj),
            P = {},
            S = {},
            C = [];
        ...
        function ah(au) {
            if (o) {
                var ap = new Date() - v,
                    ax = m(Math.round(ag + (O - I) / am), an - 1, M + 1),
                    ao = ax,
                    aq, at, ar = o.offset().top;
                au.stopPropagation();
                if (au.type === "mouseup") {
        ...
        function U(p, ay, s, aA, au) {
            var aq, ax = h('.dw-li[data-val="' + p + '"]', ay),
                az = h(".dw-li", ay),
                aw = az.index(ax),
                ao = az.length;
            if (aA) {
...
(function (d, f) {
    var b = d.mobiscroll,
        ...
        a = new Date(),
        e = {
            startYear: a.getFullYear() - 5,
            endYear: a.getFullYear() + 100,
            separator: " ",
            dateFormat: "mm/dd/yy",
            dateOrder: "mmddy",
            timeWheels: "hhiiA",
...
(function (d, f) {
    ...
    c.presets.scroller.select = function (M) {
        var j, v, A, L, D, h, O, af, k, t, q, U, H, aj, J, l = {},
            ah = {},
            C = {},
            w = {},
            ae = {},
            g = d.extend({}, M.settings),
            W = d.extend(M.settings, e, g),
            m = W.batch,
            K = W.layout || (/top|bottom/.test(W.display) ? "liquid" : ""),
            G = K == "liquid",
            ag = d(this),
            Q = W.multiple || ag.prop("multiple"),
            P = b.isNumeric(W.multiple) ? W.multiple : Infinity,
            S = this.id + "_dummy",
            y = d('label[for="' + this.id + '"]').attr("for", S),
            Z = W.label !== f ? W.label : (y.length ? y.text() : ag.attr("name")),
            u = "dw-msel mbsc-ic mbsc-ic-" + W.checkIcon,
            Y = W.readonly,
            ai = W.data,
            R = !!ai,
            ad = R ? !!W.group : d("optgroup", ag).length,
            T = W.group,
            aa = ad && T && T.groupWheel !== false,
            ac = ad && T && aa && T.clustered === true,
            B = ad && (!T || (T.header !== false && !ac)),
            ak = ag.val() || [],
            V = [],
            I = {},
            E = {},
        ...
        function F(av, an, ar, au, ap, ay, at) {
            var ao, aq, ax = [],
                aw = [],
                al = ar[au] !== f ? ar[au].index : 0,
                s = Math.max(0, al - m),
                am = Math.min(an.length - 1, s + m * 2);
            if (ah[ap] !== s || C[ap] !== am) {
                for (ao = s; ao <= am; ao++) {
                    aw.push(an[ao].text);
                    ax.push(an[ao].value)
